...
- **Orchestrator batch (broker side)**: targeted announces (`POST /announce` with `to_peer_id`, the team-lead notification path), an **operator inbox** (agents `send_message` to the reserved `operator` peer -- the human in front of the Deck -- drained via `POST /operator-inbox`; requires a group with a secret, since the secret-less `default` group could be drained by anyone), and a `queue` position on roadmap items (the Deck's dispatch queue).
...
| `CLAUDE_PEERS_APPROVAL_NOTIF_TTL_HOURS` | (n/a)                  | `24`                                 | broker                | Remote approvals: lifetime of the NOTIFICATION. Past it a pending approval flips to `expired_notif` — the session stays blocked and the Deck can still settle it. |
| `CLAUDE_PEERS_APPROVAL_TTL_DAYS`     | (n/a)                  | `30`                                 | broker                | Remote approvals: retention of SETTLED rows. Pending approvals are never purged (they wait for a human, however long). |
| `CLAUDE_PEERS_APPROVAL_MAX_PENDING`  | (n/a)                  | `200`                                | broker                | Remote approvals: max pending approvals per operator. Bounds DB growth and caps how much a compromised sandboxed agent can spam its own operator. |
| `CLAUDE_PEERS_GRAPH_DRAFT_TTL_DAYS`  | (n/a)                  | `30`                                 | broker                | Retention of OPENED graph drafts before the sweep deletes them (extend at will). Pending drafts are never purged: they wait for the operator. |

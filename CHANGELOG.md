...
**Le courrier operateur se remplissait d'une question BLOQUANTE par session, et la cause nommee par la carte etait fausse.** La carte accusait le producteur de secours du Deck ; le journal du run de l'incident ne contient AUCUNE ligne `[attention]` dans la fenetre des dix entrees, alors que ce producteur journalise systematiquement avant d'agir. Le vrai producteur est le hook, qui mappait la notification `idle_prompt` du CLI en approbation de type question. Le renversement decisif est que `idle_prompt` est emis SOUS GARDE `!isDialogOnScreen()` : c'est la garantie mesuree qu'aucune question n'est posee, donc l'exact contraire d'une question bloquante. Le mapping devient une allow-list d'un seul type, de sorte que tout `notification_type` futur tombe sur `skip` par construction ; une sonde sur un type volontairement fictif epingle cette FORME, qui ne tenait jusque-la qu'a un litteral incident dans un test que le lot n'avait pas ecrit.

**Le declencheur n'est pas le redemarrage, c'est chaque fin de tour non suivie d'une relance dans les 60 secondes.** Le seuil vaut 60 000 ms dans le binaire du CLI, et il compte depuis `lastQueryCompletionTime`, pas depuis le demarrage ni depuis l'affichage de l'invite. Le controle arithmetique le prouve : les dix entrees de l'incident sont arrivees 221 a 367 secondes apres l'acquittement des sessions, ETALEES sur 2 min 26 s. Un evenement commun ne s'etale pas ; dix durees de tour differentes, si. Le redemarrage n'est donc qu'un SYNCHRONISATEUR, qui fait finir onze tours dans la meme minute et rend spectaculaire un defaut qui coule en continu. Un zero observe en cours de matinee a semble refuter tout cela : il venait de la cadence de dispatch du lead, qui relancait chaque pair en moins d'une minute. Trois entrees sont apparues des que trois pairs ont ete mis en attente, dans l'ordre exact ou ils avaient fini.

**La garde bout-en-bout de ce correctif etait fail-open toute seule, et sa correction est le fait le plus reutilisable du lot.** Le hook sort par un `process.exit(0)` INCONDITIONNEL, donc son code de retour ne prouve rien. Deux mutations le montrent : une exception jetee en premiere ligne de `main()`, et un POST court-circuite, laissent toutes deux le test d'absence VERT alors que le hook est mort. Une assertion a zero via un service externe est vraie de trop de facons. Le correctif rend l'appariement INTERNE : le meme harnais, le meme credential et le meme broker refusent `idle_prompt` ET acceptent `agent_needs_input` dans le meme test, ce qui est verifie par rejeu des deux mutations, rouges cette fois.

**`project_key` etait stocke verbatim, et sept portes d'ecriture le refusent desormais.** Le balayage decommente -- indispensable, un commentaire contenant le nom d'un helper faisant passer un handler pour protege -- donne 18 occurrences reparties sur 11 sites, et non les quatre que la carte annoncait. Sept ecrivent et sont cablees : `/register`, `/approval/token-mint`, la branche INSERT de `/roadmap/upsert`, `/roadmap/import`, plus `/roadmap/lock-park`, `/roadmap/lock-release` et `/roadmap/reorder`, ces trois derniers ayant ete pris a tort pour des filtres de lecture alors qu'ils servent de PORTEE a un UPDATE. La preuve du defaut tient en deux requetes sur le meme broker : la meme chaine a espace de tete rendait 400 a une porte et 200 a une autre, dans le lot qui etablit la discipline. Le `slice(0,256)` silencieux du mint devient un refus : une troncature fabrique en silence une seconde cle pour un meme projet.

**Le validateur est une deny-list, et l'allow-list de charset qui semblait evidente est mesurablement fausse.** Un test vert PRE-EXISTANT epingle qu'un `project_key` accentue est la sortie CORRECTE, parce que le `LOWER()` de SQLite est ASCII-only et qu'un repli ASCII forkerait une troisieme cle en collision ; et la branche de repli de `normalizeRemoteUrl` produit legitimement antislashs, deux-points et espaces internes pour un remote de chemin local. Une allow-list derivee de l'echantillon en base ne peut que SOUS-representer le codomaine du producteur, donc elle echoue CLOSED sur la premiere forme legitime pas encore rencontree. Le docstring dit aussi ce que le lot n'achete PAS : les homoglyphes ne sont fermes ni ici ni ailleurs pour ce champ, et une premiere redaction renvoyait a un dispositif nomme "operator partitioning" qui n'existe nulle part -- la phrase a ete supprimee, un commentaire qui se donne du merite en pointant vers un mecanisme introuvable etant precisement le defaut que ce depot paie en boucle.

**Le critere de cablage vit desormais dans le docstring du validateur et non plus en liste dans un fichier de test, parce que la liste etait deja fausse le jour ou elle partait.** Elle oubliait un huitieme site lecteur, annonçait deux exemptions puis en enumerait trois, et surtout vivait la ou le prochain developpeur de `broker.ts` ne l'aurait jamais lue. Un effet de bord du meme lot a impose un second correctif : faire s'effondrer une cle invalide en NULL a l'enregistrement rouvrait la regression owner-gone, `roadmap_items.project_key` etant `TEXT NOT NULL`, donc un pair a NULL ne matche AUCUNE carte et le balayeur le lit comme proprietaire disparu -- pair vivant, battant du coeur, prive de ses propres verrous. Le declencheur etait LEGITIME et non hostile : `resolveProjectKey` n'avait aucun plafond sur sa branche remote. Le producteur borne desormais sa sortie, ce qui laisse l'effondrement aux seuls clients casses, et la mesure sur le broker vivant montre que la population re-keyee est VIDE : 83 lignes de pair, six cles distinctes, longueur maximale 28 pour un plafond de 256.

**Trois cartes ont ete perdues dans un projet fantome, et recuperees.** Le broker stocke le `project_key` VERBATIM tandis que toutes les routes de lecture travaillent sur la forme minuscule : un depot mal case reussit, rend un id credible, et la carte n'existe pour personne. L'export sur la forme minuscule rend 314 cartes, sur la forme majuscule 3. La cause racine etait une doc -- `roadmap-scribe.md` prescrivait de ne minusculer que l'hote et donnait en exemple la forme fantome elle-meme -- et un `scripts/migrate-project-key-case.ts` avait deja nettoye les donnees UNE fois le 20 aout, sans que la source du mauvais format soit fermee : les trois orphelines datent toutes d'apres cette migration. La recuperation devait imperativement PRECEDER toute normalisation a l'ingress, faute de quoi le seau fantome deviendrait inatteignable par toute route. Elle est faite, ids et auteurs preserves. La fermeture par construction passe par `cli.ts`, seul endroit ou un agent tapait la cle a la main : `roadmap-add` la derive desormais de son propre cwd et refuse un champ en desaccord. L'outil MCP, lui, etait deja correct.

**Deux retombees silencieuses laissent enfin une trace, et un docstring cessait d'etre vrai.** Quand `CLAUDE_PEERS_FORCE_GROUP_FILE` designe un fichier absent ou vide, `resolveForcedGroupSecret` rendait `null` sans un mot, et la session basculait sur le groupe `default` -- une session vide partait meme sur un groupe deterministe identique sur toutes les machines. Les deux branches tracent maintenant, la branche exception aussi, et le logger est cree avec `mirrorToConsole: false` : la protection du stdout du serveur MCP ne dependait jusque-la que du fait que les deux seules branches etaient warn/error, donc d'une garde couvrant l'etat present plutot que de la construction.

**Une garde de diffusion annoncait couvrir tous les handlers et en voyait trois sur cinq, pour une raison sans rapport avec ce qu'elle verifiait.** Son regex exigeait a la fois un receveur nomme `*Detector` et un parametre destructure, si bien qu'un site echouait sur le NOM alors que sa forme etait correcte. Quatre formes de code passaient vertes, dont un `*Tracker` qui s'etait reellement produit la veille. La garde est remplacee et non elargie : elle balaye desormais tout `.on(` du fichier et LEVE une erreur explicite quand elle ne resout pas une reference nommee, au lieu de traiter le cas comme sans danger. Au passage, les trois branches de rendu de l'etat ternaire livre la veille etaient supprimables avec ZERO rouge sur 1369 tests ; deux d'entre elles sont desormais couvertes, la troisieme est cartee avec son blocage mesure.

**Six glyphes de role sont livres, et trois d'entre eux ont ete redessines apres la premiere passe ecran.** Le typecheck etait vert, les tests verts, et une revue de code n'aurait rien vu : a 13 pixels, le marteau se lisait comme la lettre T, le compas comme la lettre A, la toile se remplissait en tache, et la pelote se confondait avec la loupe de recherche. Un glyphe dont la silhouette EST une lettre est le pire cas dans un jeu d'icones. Le token de la bande repliee passe de 58 a 70 pixels, somme exacte de ses occupants et non reserve -- une reserve est une promesse que personne ne recalcule. Le glyphe se pose APRES `.row-main` et non dans la fente que le code lui reservait : la mesure montre que la fente decalait le nom de 21 pixels sur les seules lignes porteuses d'un role, soit exactement le defaut d'alignement qu'un lot precedent venait de supprimer.

**La carte `a688748b` accusait une contention de ports, et cette premisse est refutee par mesure.** Un A/B sur les 59 fichiers appelant `startBroker`, application ouverte, rend 557 pass et 0 fail en 153,70 s avant correctif et 153,79 s apres : faire attribuer le port par le systeme ne change rien. La cause reelle est une collision d'export entre deux `mock.module` du meme specifier `desktop/src/renderer/src/store.ts`, dans `tests/desktop-session-broadcast.test.ts` et `tests/desktop-activity-unknown-render.test.ts` : Bun fige la liste d'exports d'un module mocke au premier module materialise, pour tout le processus. Le second fichier meurt au chargement sur `SyntaxError: Export named 'errorText' not found in module store.ts`, APRES son `GlobalRegistrator.register()` et avant son `afterAll(unregister)`, ce qui laisse l'enregistrement happy-dom ORPHELIN : `globalThis.fetch` reste celui de happy-dom pour tout le reste du processus, et la Same-Origin Policy refuse la reponse du broker de test sur `/health` (le broker repond, sans en-tete `Access-Control-Allow-Origin`), jusqu'a epuisement des tentatives de la sonde de demarrage. Consequence economique : 188 s deviennent 1503 s. Le correctif tient en une cle ajoutee au mock. `tests/_helper.ts` gagne quand meme un port attribue par le systeme au lieu d'une fenetre partagee incrementee sequentiellement, ce qui fait passer la boucle externe de vingt a trois tentatives -- une fragilite reelle mais DISTINCTE, qui ne corrige pas la degradation mesuree ici.

**L'echantillon A/B etait aveugle a l'hypothese par construction, ce qui merite d'etre consigne comme methode.** L'intersection entre les 59 fichiers appelant `startBroker` et les 10 fichiers appelant `GlobalRegistrator.register` est VIDE : le defaut exigeait la PAIRE des deux, donc aucun grep sur un seul des deux marqueurs ne pouvait le voir.

**Ce que ce lot NE change PAS, et il faut le dire.** L'application tourne sur le build du 26 aout a 23:18, et `app.asar` embarque tout `desktop/src` : rien de ce qui touche le desktop ici n'atteindra l'ecran avant un rebuild. La chaine de deploiement fait DEUX sauts et non un, le second etant la copie empaquetee sous `dist/win-unpacked/resources/` -- un correctif construit a mi-parcours a laisse le produit charger l'ancienne version pendant vingt minutes, et le mapping se trouvant a la ligne 101 dans les DEUX fichiers, un reviewer a lu la bonne ligne du mauvais fichier et conclu a tort que le correctif etait deploye.

**Sept cartes deposees et cinq enrichies.** `51fd7b65` (la perte silencieuse par casse, `must`), `7172f416` (une approbation levee par le hook n'a aucun chemin d'abandon), `acda9b59` (aucune sortie hors reponse dans le courrier, avec la decouverte que le statut `abandoned` existe deja et rend le lot deux fois plus petit), `fbaf35df` (les artefacts construits derivent de leurs sources, corrigee en cours de route : l'unite est le BUILD et non l'artefact), `a3ab9f7f` (`StopState.unknown` lu par personne), `b7a0c658` (la branche ternaire de `BrowserView` reste decouverte, avec son blocage mesure), `38029bfb` (au-dela d'une vingtaine de sessions la liste du volet Agents deborde au lieu de defiler).

**Deux cartes supplementaires deposees pendant le diagnostic de contention de ports.** `33469d48` (217 fichiers de test sur disque, 163 joues par le runner supporte, 54 exemptes sans executeur autre que le `bun test` nu -- exactement le mode que la contamination ci-dessus vient de demontrer fragile) et une garde structurelle sur les `mock.module` a surface d'export incomplete, encore en ecriture par un autre agent et non livree ici.

**Deux gardes contre la rechute du mock incomplet** (carte `a688748b`). `tests/_store-mock.ts` expose `mockStore()`, qui derive la surface d'export requise en scannant la source de `store.ts` elle-meme au lieu d'enumerer les fichiers consommateurs : six tests l'adoptent. Une seconde passe ferme le residu de couverture du premier lot, ou seuls les call sites ADOPTANT le helper etaient proteges : `findDirectStoreMocks()` balaie tout `tests/` via le `listTestFiles()` partage avec `scripts/pure-module-partition.ts`, et refuse tout `mock.module(store.ts)` ecrit hors du fichier canonique. Domaine derive du repertoire, donc un fichier neuf y entre sans que personne n'y pense : le balayage a d'ailleurs trouve lui-meme un sixieme fichier que personne n'avait vu, `tests/desktop-inbox-sender-dom.test.ts`, dont le mock direct etait complet mais hors du chemin verifie. Les deux angles morts restants, specifier construit dans une variable et specifier atteint par un alias de chemin, sont documentes en commentaire ET epingles par un test, plutot que muets.

**Une revue par mutation sur les deux gardes ci-dessus a rendu un bloquant et quatre trous, tous fermes dans la foulee.** Le bloquant : le domaine de `findDirectStoreMocks()` etait `listTestFiles()`, qui ne retient que `*.test.ts` -- un fichier HELPER (`_evil-helper.ts` chez le reviewer, exactement la forme de `_helper.ts`/`_store-mock.ts` deja reels) mockait `store.ts` sans jamais courir comme test et restait invisible ; le domaine devient tout `tests/*.ts` via `listTestsDirFiles()`, mesure identique a un `readdirSync` independant plutot qu'un plancher de taille (qui, mesure, restait vert sur un domaine tronque de 218 a 101 fichiers). Trois formes d'export narrowaient `readRealStoreKeys()` en silence sans qu'aucun test ne bouge -- `export class`, un re-export `export { x }`, et `export function` reflowe sur deux lignes -- desormais toute ligne `export` non reconnue leve une erreur au lieu de disparaitre discretement ; un cinquieme mutant (`export const enum`) capturait a tort le mot `enum` comme cle bidon, corrige par une branche dediee plus un lookahead. L'exemption du fichier canonique etait du CODE MORT sur le vrai chemin d'appel (le specifier passait par une variable, invisible au scan qui n'accepte qu'un litteral) : `mockStore()` ecrit desormais le litteral en dur, ce qui rend l'exemption reellement exercee. Batterie rejouee sur miroir jetable, jamais sur l'arbre partage : cinq mutations rouges, une (le faux positif `enum`) redevenue verte.

**Un second rejeu independant a trouve trois trous supplementaires dans le correctif ci-dessus, tous fermes.** Une ligne RECONNUE peut encore perdre une cle : `export const tone = 1, badge = 2` ne capturait que le premier declarateur et jetait `badge` en silence -- meme classe que les mutations precedentes, un cran plus bas (extraction incomplete d'une ligne reconnue, pas une ligne non reconnue) ; `readRealStoreKeys()` rejette desormais toute ligne `const` multi-declarateur. Le filtre d'extension `.ts` laissait passer un helper `.tsx`, forme plausible dans un repertoire qui monte du React sous happy-dom : elargi a `.ts`/`.tsx`/`.mts`/`.cts`. Et `listTestsDirFiles()` n'etait pas recursif -- un premier correctif JETAIT sur toute sous-repertoire, ce qui aurait rendu la garde perpetuellement rouge contre ce depot meme, puisque `tests/fixtures/` et `tests/pty-harness/` existent deja : remplace par un `readdirSync` recursif, qui ferme la croissance du domaine PAR CONSTRUCTION plutot que par une assertion a tenir a jour. Point de methode retenu du reviewer : le plancher de taille et l'egalite contre un `readdirSync` independant ne sont PAS redondants -- l'egalite attrape une derive de l'IMPLEMENTATION, le plancher attrape une derive du DOMAINE (`TESTS_DIR` deviant vers un autre repertoire, ou les deux cotes de l'egalite s'accordent a tort sur la meme mauvaise cible).

Etat : porte complete rejouee SANS fermer l'application, Koryphaios ouvert avec ses sessions vivantes et son processus Electron -- **2684 passants, 0 echec, 2 sautes, 215 fichiers, 202,12 s** (reference en bon etat : 188 s). Les deux passages a 2635 passants, 28 echecs, 19 erreurs, 1503 s consignes precedemment etaient une porte POLLUEE par le defaut diagnostique et corrige ci-dessus (carte `a688748b`) : les trois empreintes de cette contamination -- `Cross-Origin Request Blocked`, `could not start broker on any port`, `already been globally registered` -- comptent desormais a zero dans cette sortie. Le seul defaut REEL trouve par cette porte restait dans une garde et non dans un lot : son demi-test au niveau fichier utilisait un `includes` nu, sans la discipline de decommentage que son demi-test au niveau statement applique, si bien qu'elle punissait le fait de DOCUMENTER la table qu'elle protege ; les deux moities partagent desormais le meme predicat. Build de fumee vert, trois entrypoints. `npm run typecheck` dans `desktop/` sorti en 0.

Fichiers : `desktop/src/main/detect/activity.ts` (nouveau), `shared/graph-draft-scope.ts` (nouveau), `desktop/src/main/detect/osc.ts`, `session-service.ts`, `thinking.ts`, `ipc.ts`, `deck-control.ts`, `i18n.ts`, `desktop/src/shared/types.ts`, les trois composants de rendu, `styles.css`, les deux locales, `broker.ts`, `server.ts`, `docs/DESIGN-ACTIVITY-PREDICATE.md`, `TESTING.md`, et huit fichiers de tests. Cartes `f8082208`, `3781b033`, `0bbac537`.

**`SessionRuntime.thinking` etait CONSTANT `false` en production, et le champ est desormais un etat TERNAIRE `working`/`idle`/`unknown` produit par la FREQUENCE des emissions de titre.** Le cablage aval etait vivant, la source etait morte. Un compteur monotone `titleSeq` est ajoute a `OscSnapshot`, un module pur `detect/activity.ts` en derive l'etat, et le champ a ete RENOMME plutot que retype : le test de verite de `waitIdle` sur une union de litteraux de chaine ne produit aucune erreur du compilateur, donc changer le type en place aurait bascule la porte d'injection EN SILENCE. Le renommage fait echouer la compilation sur chaque consommateur et rend l'inventaire gratuit. `waitIdle` est repointe DANS LE MEME COMMIT sur la recence d'octets et non sur le nouvel etat : le titre est muet pendant la frappe, donc l'y brancher aurait transforme une porte toujours ouverte en porte ouverte exactement pendant que l'operateur tape, defaut intermittent ayant l'apparence d'une reparation.

**Deux tours de revue par mutation ont produit l'essentiel de la valeur, et le module pur n'y etait pour rien : ses quatre mutations internes etaient deja rouges au premier tour.** Ce qui manquait etait la preuve du CABLAGE. Sur les six sites capables d'annuler le ternaire, UN SEUL mordait. Le sixieme n'etait meme pas dans le brief du lot : la sortie de session ecrivait `idle` en dur, donc une session n'ayant jamais peint de titre y devenait `idle`, exactement le repli que le lot existe pour interdire. Supprimer l'ecriture du champ, ou sa diffusion, laissait la suite verte. Et `titleSeq` pouvait se mettre a compter TOUTES les sequences, y compris les barres de progression que le CLI emet vraiment, sans qu'un test bronche : les sondes prouvaient que les titres comptent, aucune ne prouvait que le reste ne compte PAS. Chaque correction est prouvee par sa mutation rejouee rouge puis revertee, hash du fichier verifie identique.

**La portee et l'auteur d'un brouillon de graphe sont desormais derives de la ligne `peers` prouvee, au lieu d'etre crus sur declaration du corps de la requete.** Le seul rempart etait le `BROKER_TOKEN`, que tout agent spawne detient. Quatre gestes prouves sur broker isole avant correctif : ecriture inter-projets ATTRIBUEE A L'OPERATEUR, lecture inter-projets complete, IDOR destructif sur `/graph-draft/open` qui CONSOMME le brouillon de la victime, et usurpation d'affichage par homoglyphe stocke verbatim. Sur le MEME broker, la meme revendication d'identite sur `/roadmap/upsert` repond 401 : l'ecart etait un oubli, pas un choix. `resolveProvenGraphDraftPeer` exige l'`instance_token` d'un peer enregistre et le handler n'accede plus jamais a `body.by` ni `body.project_key`, donc l'usurpation d'ATTRIBUTION devient impossible par construction plutot que filtree, garantie plus forte que celle specifiee au depart et obtenue en simplifiant.

**Ce commit ferme la ROUTE et non la FAMILLE, et il le dit.** Trois handlers de lecture restent ouverts, et la PORTEE elle-meme reste declarative un cran en amont : un attaquant appelle `/register` en declarant le `project_key` de la victime, puis depose legitimement dans son inbox. La carte `c92614ed` porte la fermeture par le type des deux tables. Deux detours de conception ont ete imposes par des gardes qui disaient vrai : la parite `DECK_WRITE_ROUTES` a refuse le premier resolveur, trop large pour la question posee, et la garde de la carte `b33b1874` a refuse le premier emplacement du test. Les deux sorties evidentes etaient fermees, renommer la garde en `broker-*` l'aurait rendue verte en la RETIRANT de la CI, d'ou l'injection de dependance sur le modele de `shared/approval-scope.ts`. La preuve est livree en deux moities, la decision en CI par un double injecte, le cablage en local par le vrai handler HTTP, parce qu'extraire une decision dans un module pur rend son site d'appel invisible.

**Deux tests etaient verts uniquement depuis une tuile Koryphaios, pour deux raisons differentes, et les deux sont consignees dans `TESTING.md`.** L'unique echec de la porte du matin venait d'un `spawnPeer` qui n'epinglait pas son groupe : sans `CLAUDE_PEERS_FORCE_GROUP`, la resolution retombe sur `default`, que le broker refuse pour un depot dans l'inbox operateur. Le Deck exportant lui-meme la variable, le test passait ici et echouait partout ailleurs ; son fichier jumeau portait deja la discipline et l'enoncait en tete, donc l'exception etait le bug. Le second cas est la CI : la garde qui epingle les etapes de typecheck du workflow n'a JAMAIS ete verte sur `windows-latest` depuis sa naissance, `actions/checkout` y appliquant `autocrlf` sur un depot en LF, ce qui desarme un regex ancre sur un saut de ligne nu. La restriction mesuree est plus etroite que la regle intuitive et elle est ecrite comme telle : seul un motif dont le saut de ligne precede immediatement un token attendu casse. Un candidat voisin a ete mesure IMMUNE et n'a ete durci que par prudence, son commentaire le precisant pour ne pas faire croire a un defaut inexistant.

**La carte `0bbac537` n'est pas fermee, elle est refutee et annotee.** Elle affirmait une CI rouge en continu depuis le 18 aout : sept runs verts existent apres cette date, et le rouge d'aujourd'hui n'a AUCUN nom de test en commun avec celui qu'elle decrit. Un compte d'echecs comparable masquait un ensemble entierement different, ce qui est precisement le piege que fige une carte redigee par un NOMBRE plutot que par un ENSEMBLE. Le second rouge du jour est un flake de temporisation, prouve par repetition sur trois points de CI et trois runs locaux.

**Cinq cartes deposees pour des trous adjacents non fermes ici.** `c92614ed` (la portee `project_key` n'est prouvee sur aucune table, et `/register` l'accepte sans preuve), `581a0d56` (la garde de diffusion annonce couvrir tous les handlers et en voit trois sur cinq, couverture en RECUL du fait de ce lot meme), `fbb0ef72` (la garde SQL enumere deux fichiers en dur, et aucune garde ne parcourt `shared/`), `47baf25a` (une session simplement posee a son invite leve une question BLOQUANTE dans le courrier operateur, dix entrees pour dix sessions apres un redemarrage), `3e5f1853` (un `CLAUDE_PEERS_FORCE_GROUP_FILE` pendant ferait retomber une session sur `default` en silence).

Etat : porte complete passee sur l'arbre final -- 2639 tests passants, 0 echec, 2 sautes, 213 fichiers, 187,65 s. Build de fumee vert (244 modules). `npm run typecheck` dans `desktop/` sorti en 0. Le test du groupe est verifie vert AVEC l'environnement depouille, donc la porte vaut hors d'une tuile Koryphaios. Carte `f8082208` livree, `3781b033` livree cote ecriture. Propagation requise : le lot touche `desktop/src/main`, l'app doit etre rebuildee ; `broker.ts` et `server.ts` ont bouge, donc le clone MCP et le broker doivent etre redemarres.

## desktop/docs -- le plafond de recuperation d'osc.ts fuyait la sequence suivante, l'instrument de capture PTY brute est livre, et l'arbitrage CONTENU-contre-FREQUENCE du predicat d'activite est tranche

Fichiers : `desktop/src/main/detect/osc.ts`, `tests/desktop-osc.test.ts`, `desktop/src/main/pty-manager.ts`, `docs/DESIGN-ACTIVITY-PREDICATE.md` (nouveau). Cartes `5b324e11`, `00588e6c`, `f8082208`.

**`osc.ts` posait un flag `capped` au depassement du plafond mais laissait `mode` bloque en `in-osc` ou `in-osc-esc-seen`, contrairement a son jumeau `safe-strip.ts` qui sort en `idle` immediatement.** Consequence mesuree : la sequence OSC bien formee SUIVANTE etait absorbee dans la recherche de terminateur de la tete abandonnee, et son propre terminateur ne declenchait pas `applyPayload` puisque `capped` restait vrai. Perte silencieuse, sans erreur ni trace, sur un module que la carte `f8082208` s'apprete a brancher a un consommateur de production. `appendAndCheckCap` remplace `pushChar`/`capped` et repasse en `idle` des le depassement ; le terminateur ST 8 bits (0x9C) est desormais accepte, en miroir de `safe-strip.ts`.

**Trois sondes mesurees ROUGE avant correctif shippent dans le diff.** Tete OSC sur-longue jamais terminee suivie d'une sequence valide sans BEL intermediaire, meme mecanique avec un ST 8 bits, et depassement declenche depuis `in-osc-esc-seen`. Cette troisieme transition est sortie d'une revue par mutation : le mutant qui ignore la valeur de retour de `appendAndCheckCap` a ce site d'appel restait vert contre la suite entiere, alors que le code, lui, etait correct -- le trou etait dans la preuve, pas dans la logique.

**`KORY_PTY_RAW_CAPTURE` livre l'instrument de mesure, pas la mesure : les motifs `\s*` de `startup-ack.ts` et `attention.ts` encodent l'espacement du repaint ConPTY sur la seule foi d'un commentaire dont le log source est perdu.** Definie sur un chemin absolu hors depot, la variable ecrit les trente premiers chunks PTY en hexadecimal des octets UTF-8 exacts, plus leur forme echappee. Non definie, aucun appel au systeme de fichiers. Le point d'accroche est `proc.onData` dans `pty-manager.ts`, AVANT `handleData`, donc avant le retrait des marqueurs et toute heuristique -- le point le plus amont joignable depuis notre code, condition de validite de la capture : plus bas, elle mesurerait notre propre pipeline au lieu de l'encodage reel de ConPTY. Reste ouvert en phase deux, non-code : la capture reelle sur une session operateur, et le remplacement du commentaire orphelin par une reference a un log conserve.

**`docs/DESIGN-ACTIVITY-PREDICATE.md` tranche le point que le depot portait comme explicitement non tranche depuis le matin meme : la FREQUENCE decide, le CONTENU decore, mais pas pour le motif que trois cartes portaient comme acquis.** Une mesure neuve sur les fixtures deja versionnees etablit que le contenu n'est pas aveugle -- le titre OSC 0 porte une vraie animation de travail en demi-cercles, presente dans la fenetre du tour ; ce qui est aveugle, c'est le glyphe de repos, emis dans toutes les classes. La frequence l'emporte quand meme par le MODE DE MORT : un capteur de contenu meurt en rendant faux pour toujours sans un rouge, un taux meurt en faisant scintiller le badge, visible le jour meme -- ce depot a deja paye trois fois la premiere classe et zero fois la seconde. Le document impose l'etat TERNAIRE (`working`/`idle`/`unknown`) : un booleen rendrait faux pour toujours un agent-kind qui ne peint pas de titre, badge jamais affiche, zero rouge. Le document ecrit aussi le piege qui vise le prochain commit : rendre le champ d'activite vivant sans repointer `waitIdle` ne le laisse pas en l'etat, il le rend intermittent (ouvert exactement pendant la frappe, le titre etant muet), avec l'apparence d'une reparation.

**Mesure faite cette session, qui ferme une question laissee ouverte par trois cartes : `SessionRuntime.thinking` est CONSTANT `false` en production.** Le cablage aval est vivant et complet (le commit `62e7330` a bien livre la diffusion), c'est la SOURCE qui est morte : `BUSY_RE` (`desktop/src/main/thinking.ts`) cherche du braille, alors que les vrais spinners du CLI sont U+2736 et voisins. Rejouer les 7 fixtures PTY reelles du depot a travers le vrai `ThinkingDetector` donne zero evenement, y compris sur deux tours complets, alors que la meme fixture porte 10 emissions de titre OSC 0. Quatre consommateurs sont cables sur cette valeur fausse : `waitIdle` (la porte d'injection de directive est donc en permanence ouverte), le compte `agents:stop-state` (toujours zero), la projection MCP `deck-control`, et le badge visuel. Consequence : la carte `fda382b3` (H6, le badge "termine pendant que vous ne regardiez pas") reste BLOQUEE, exactement comme son propre briefing l'instruisait, plutot que de livrer un habillage sur un signal mort.

**Deux cartes deposees pendant ce lot pour des trous adjacents non fermes ici :** `496ed818` (`screen-model.ts` porte un TROISIEME scan OSC, entierement sans etat entre deux chunks, avec un consommateur de production vivant puisque `screenGuard.classify` alimente `injectCommand` et `interrupt`), et `1dd4e95c` (`safe-strip.ts` n'incremente pas `pendingLen` sur sa porte ESC, le bug d'avalement permanent reste ouvert la). Notes de levee deposees sur les cartes `f8082208` et `8691dea3`.

Etat : gate complete passee une fois sur l'arbre final -- 2603 tests passants, 0 echec, 2 sautes, 211 fichiers, 188,65 s. Build de fumee vert (`bun build --target=bun broker.ts server.ts cli.ts`). `npm run typecheck` dans `desktop/` sorti en 0. Carte `5b324e11` fermee. Carte `00588e6c` reste ouverte en phase 2.
...
**Un depot hostile obtenait l'execution d'une commande sur le shell de l'hote, au clic Restore.** Les espaces de travail sont lus depuis `<projectDir>/.claude/claude-peers/workspaces/*.json`, donc depuis le depot CLONE. Le champ `args` y etait colle verbatim dans la ligne shell de lancement, sans echappement et sans aucune porte, alors que les TEMPLATES exigeaient deja une approbation pour exactement ce meme sink -- le commentaire de `template.ts` le disait noir sur blanc, "appended verbatim to the login-shell command line", et `ipc.ts` assumait l'inverse cote workspace, "Restore respawns straight through SessionService (no create gate)". Meme contenu, meme depot, une porte sur deux. C'etait un oubli, pas un choix : le mecanisme amont qui l'aurait excuse a ete cherche et n'existe pas.

**Reproduit jusqu'a l'argv avant d'etre corrige, et le declencheur etait garanti par l'attaquant.** Un point-virgule place dans `args` survivait intact dans le payload de `sh -l -c` comme dans celui de `powershell -Command`, et `pty-manager.ts` spawne cet argv tel quel. On pourrait croire la restauration sure parce qu'en mode `resume` les args sont droppes : elle ne l'etait pas, `spawnSession` bascule en `fresh` des qu'il n'y a pas de transcript, donc un `claudeSessionId` bidon suffisait. Un correctif qui aurait force le mode `resume` aurait ete contourne par une ligne de JSON.

**Le correctif est une PORTE, pas un assainisseur, et c'est delibere.** `buildSessionCommandLine` est inchange et injecte toujours si on l'atteint : c'est le sink partage avec les templates, qui l'utilisent AVEC approbation. La faille se ferme parce que `restore()` refuse AVANT de l'atteindre. Durcir le builder aurait fait diverger les deux chemins. `sessionsHaveShellFields` factorise le predicat que `templateHasShellFields` portait seul, et le dialogue B4 est factorise en un point unique reutilise par les deux portees : aucune seconde regle n'a ete ecrite, precisement parce que ces deux-la ont DEJA diverge une fois, sur le champ `lead`.

**LE PREMIER ARBITRAGE ETAIT FAUX, ET C'EST LA LECON LA PLUS TRANSPOSABLE DU LOT.** Le maillon `cwd` avait ete ecarte au motif qu'il etait redondant : l'execution de commande par `args` dominait la lecture de fichiers par `cwd`. Cet argument tenait AVANT le correctif et est mort AVEC lui. Une fois `args` gate et `cwd` non, la lecture n'etait plus dominee par rien et devenait le seul vecteur non gate livrable par un depot clone. Regle a retenir : tout arbitrage qui ecarte un vecteur au motif qu'il est domine doit etre REJOUE apres le correctif qui ferme le dominant.

**Deux classes de danger, donc deux predicats et deux cles, jamais un seul.** `workspaceHasUntrustedCwd` est distinct de `workspaceHasShellFields` : melanger execution de commande et lecture de fichier dans un meme predicat aurait conflate deux choses qui ne se refusent pas pour les memes raisons. Les deux portes s'appliquent en sequence et aucune ne court-circuite l'autre -- mesure : un workspace portant les deux, avec `args` approuve et `cwd` decline, est refuse. Les cles d'approbation sont namespacees (`workspace` et `workspace-cwd`), donc approuver l'une n'approuve pas l'autre, et chaque hash couvre son propre champ, donc un fichier modifie par le depot redemande une approbation.

**La containment generale a ete essayee puis ECARTEE SUR MESURE.** Contenir `cwd` a l'entree de `workDirRoots` aurait ferme la classe entiere, mais `create-session.ts` documente qu'un cwd hors arbre choisi a la main par l'operateur "stays legitimate and untouched" : la containment y aurait casse un usage reel. Le decoupage retenu est donc creation-libre contre restauration-gatee, et le controle vit dans le chemin de restauration seul. Consequence assumee : un worktree cree a la main hors du projet declenche une approbation, une seule fois, liee au hash. Les worktrees du Deck vivent sous `<projectDir>/.worktrees/` et passent sans prompt.

**Fail-closed verifie jusqu'aux cas ou ce genre de garde meurt d'habitude.** Les deux confirmations sont des dependances REQUISES et non optionnelles-defaultees ; fermer la fenetre ou annuler refuse ; une exception du dialogue se propage sans que le sink soit atteint. Le containment a ete mesure sur chemin inexistant, remontee `..`, chemin relatif, variante de casse Windows et lien symbolique reel : aucun contournement, et le repli `resolve` d'un chemin inexistant echoue en SECURITE puisqu'il ne resout pas les liens et ne peut donc faire paraitre un chemin que plus dehors, jamais plus dedans. `canonicalPath` etait exporte sans aucun appelant de production avant ce lot ; c'est son premier usage reel et il correspond a son contrat.

**Chaque maillon a ete mesure ROUGE avant son correctif, et les deux sondes vivent dans le commit.** `restoreFrom` n'a qu'un seul appelant runtime, situe dans `restore()` apres le controle, et `restore()` n'a qu'un appelant : tout chemin du fichier vers le sink shell passe par les portes. Il n'y a pas d'auto-restauration au demarrage.

**Deux residus prospectifs, nommes ici pour ne pas se perdre.** Les deux predicats forment une deny-list de champs dangereux, donc un TROISIEME champ dangereux ajoute demain a un workspace passerait en silence : une allow-list echouerait ferme, et c'est la forme a viser. Par ailleurs le refus par exception du dialogue remonte au handler IPC awaite, donc l'invoke rejette cote renderer au lieu d'un refus propre -- bruyant, jamais silencieux, mais perfectible.

## desktop -- les trois hooks du plugin entrent enfin dans le typecheck, qui ne les avait jamais lus

Fichiers : `desktop/tsconfig.node.json`, `desktop/hooks/roadmap-guard-hook.ts`. Carte `6eacc666`.

**Le vert etait une absence de couverture, pas une validation.** Ni `tsconfig.node.json` ni `tsconfig.web.json` n'incluaient `hooks/**`, alors que les trois sources de hooks du plugin (`approval-hook.ts`, `desk-backchannel-hook.ts`, `roadmap-guard-hook.ts`) sont construites et livrees dans `deck-plugin/`. `npm run typecheck` rendait exit 0 sans jamais les ouvrir. C'est la forme canonique du fail-open decrite dans `CLAUDE.md` : la degradation rend un SOUS-ENSEMBLE (rien) au lieu d'une erreur, et le resultat vert se lit comme un feu vert. `approval-hook.ts` tournait en production et n'avait jamais ete typecheck de sa vie.

**Le trou etait large, le lot est petit, et c'est la mesure qui l'a etabli avant tout chiffrage.** Couvrir `hooks/**/*.ts` fait remonter trois erreurs seulement. Deux sont des `TS6307`, qui ne sont pas des erreurs de type mais la frontiere d'include fermee, levees en ajoutant les deux fichiers de `shared/` reellement importes (`approval-client.ts` par `approval-hook.ts`, `peer-cache.ts` par `desk-backchannel-hook.ts`). La troisieme est reelle : `roadmap-guard-hook.ts` assignait `match[1]`, de type `string | undefined` sous `noUncheckedIndexedAccess`, a un `targetField: string`. `approval-hook.ts`, le fichier le plus expose, rend zero erreur.

**Le `!` ajoute est une assertion juste, pas un baillon.** Le groupe capturant du motif est `([a-zA-Z0-9_]+)` : ni optionnel, ni quantifie, ni dans une alternative. Si `exec` rend un match, le groupe 1 a necessairement participe. C'est un artefact pur de `noUncheckedIndexedAccess` et non un cas non traite. La mutation inverse le confirme : revenir a `match[1]` rend `TS2322` immediatement.

**Un GLOB pour les hooks, une enumeration pour `shared/`, et la mesure a renverse l'intuition sur le second point.** `hooks/**/*.ts` est un glob delibere : un quatrieme hook ajoute plus tard doit etre couvert sans que personne y pense, et une enumeration des trois fichiers aurait rejoue exactement le defaut corrige. Pour `shared/`, le glob `../shared/**/*.ts` a ete essaye puis REJETE sur mesure : il fait remonter sept erreurs (`config.ts`, `logger.ts`, `summarize.ts`), toutes causees par l'absence de `@types/bun` dans le projet, aucune dans les hooks. L'enumeration manuelle reste donc en place -- et contrairement a ce qu'on attend d'une enumeration, **elle echoue FERME**. La cause est `composite: true` : sous composite, un import vers un fichier non liste produit un `TS6307` bruyant qui NOMME le chemin manquant. Verifie par mutation sur les deux voies, l'import direct depuis un nouveau hook et la croissance d'un fichier deja liste qui se met a importer un troisieme. Le glob n'aurait donc rien ferme de plus.

**La couverture a ete prouvee par mutation, pas par le code de sortie.** Cinq mutations rejouees sur un miroir hors depot, cinq qui mordent : retirer le glob rend le typecheck aveugle a une vraie erreur injectee dans `approval-hook.ts` (`exit 0`, et `--listFilesOnly | grep -c hooks` = 0) ; une erreur deliberee dans chacun des deux autres hooks passe au rouge ; un quatrieme fichier cree ex nihilo dans `hooks/` est pris par le glob sans toucher la config ; retirer l'une ou l'autre des deux entrees `shared/` ramene le `TS6307` correspondant, donc aucune n'est speculative. Le `exit 0` seul aurait ete precisement le faux vert que la carte denonce : la preuve retenue est `tsc --listFilesOnly -p tsconfig.node.json | grep hooks/`, qui liste les trois fichiers.

**Le gate n'est pas decoratif, et il a un corollaire a connaitre.** `.github/workflows/desktop-build.yml` execute `npm run typecheck` dans `desktop/` sans `continue-on-error` : les hooks sont desormais reellement gardes en CI. Consequence mecanique a anticiper : sous `types: ["node", "electron-vite/node"]`, un futur hook ecrit en API Bun (`Bun.*`) partira ROUGE. C'est un echec ferme, donc acceptable, mais il surprendra.

**Le trou jumeau reste ouvert et est carde.** `desktop/mcp/deck-control-mcp.ts` et `desktop/mcp/demo-browser-mcp.ts` ne sont dans aucun programme non plus, alors qu'ils sont bundles par `build:mcp` et livres dans le plugin -- et `deck-control-mcp.ts` est la surface par laquelle un agent spawne pilote le Deck. Meme profil que `approval-hook.ts`, meme correctif pressenti (`mcp/**/*.ts` dans le meme include). Carte `a7822bc4`, non traitee ici parce que son cout n'a pas ete mesure.

## core + desktop -- une session pair porte desormais un role optionnel, pose par l'operateur et pousse jusqu'au broker

Fichiers : `broker.ts`, `server.ts`, `cli.ts`, `shared/types.ts`, `desktop/src/shared/role.ts` (nouveau), `desktop/src/shared/types.ts`, `desktop/src/main/session-service.ts`, `desktop/src/main/store.ts`, `desktop/src/renderer/src/components/CreateMenu.tsx`, `desktop/src/main/i18n.ts`, `desktop/locales/en.json`, `desktop/locales/fr.json`, `tests/broker-register-role.test.ts` (nouveau), `tests/desktop-session-role-env.test.ts` (nouveau), `tests/role-domain-sweep.test.ts` (nouveau). Carte `a2f61172`.

**Le role est une propriete du LANCEMENT, pas un etat persistant.** Il vient de `CLAUDE_PEERS_ROLE`, une variable d'environnement emise par le Deck au spawn (`session-service.ts`), et ecrase la valeur stockee a CHAQUE `/register`, reprise dormante comprise -- sans comparaison ni refus. Un transport vide ou absent normalise en `NULL` cote broker (`normalizeRole` : trim, minuscule, validation contre `ROLE_REGEX`) et c'est une DECLARATION d'absence, non une absence d'information : la cle `CLAUDE_PEERS_ROLE` est toujours emise par la session, jamais omise.

**Une conception write-once a ete implementee puis retiree, et la mesure qui l'a tuee vaut d'etre gardee.** La regle refusait toute re-inscription portant un role different de celui deja stocke. Elle butait sur un fait de la reprise dormante : celle-ci ne concerne qu'UNE session par `(host, cwd, group_id)`, la premiere inscrite, parce que la branche de collision de `session_key` mint un peer neuf sans toucher la table des sessions. Sur douze tuiles d'une fenetre partageant le meme repertoire, onze obtiennent donc toujours le role demande au lancement, et pour la douzieme le write-once faisait la mauvaise chose : il jetait le role choisi par l'operateur au profit de celui d'une session anterieure. L'argument qui tranche : heriter de `peer_id` et `instance_token` sert l'intention de la reprise, une session retrouve son identite, tandis qu'heriter du role contredirait une intention explicite exprimee au spawn -- une difference de nature, pas de degre.

**La garantie a change de nature en consequence, et se teste par balayage plutot que par scenario.** Elle n'est plus "une re-inscription ne peut pas changer un role" mais "aucun chemin atteignable par l'agent ne peut poser ou modifier un role". `tests/role-domain-sweep.test.ts` balaie le code reel de `server.ts` et `broker.ts` : aucun `inputSchema` d'outil MCP ne declare de champ role, et toute instruction SQL ecrivant a la fois `peers` et `role` n'a de site d'appel QUE dans le handler d'inscription `handleRegister`.

**Modele de menace explicite, delibrement non surdeclare : reserve par role, non resistant a un agent deliberement hostile.** Un agent disposant d'un shell peut poster une inscription HTTP directement vers `/register` et y placer le role de son choix ; c'est un cas explicitement exclu du perimetre, jamais qualifie de "securise".

**Le role est un attribut, jamais une cle.** Deux pairs du meme groupe peuvent porter le meme role, et une recherche par role rend une liste. L'unicite d'un `team-lead` reste un fait d'usage de l'operateur, jamais une contrainte codee -- rien dans `broker.ts` ne l'impose pour `role`.

**Cote interface, un menu deroulant dans la creation avancee de session, entre la couleur et le modele.** Une entree "Other..." ouvre une saisie libre dont la valeur est normalisee (`sanitizeRole`, kebab-case, 32 caracteres) et persistee dans `roleChoices`, sur la configuration GLOBALE de l'operateur (jamais un fichier de projet, puisqu'un role est un geste operateur et un depot clone ne doit pas pouvoir en suggerer un). Cocher le laurier team-lead SUGGERE le role `team-lead` tant que le champ n'a pas ete touche, sans jamais le deriver : les deux valeurs restent independantes. Aucune regle CSS nouvelle n'a ete necessaire pour le controle, les regles au niveau element (`select`, `select option`) le couvraient deja.

**Un piege de plateforme mesure dans les tests, utile a quiconque relira ce code.** Sur Windows, poser une variable d'environnement enfant a la chaine vide la SUPPRIME : `CLAUDE_PEERS_ROLE` vaut alors `undefined` a la lecture sur Windows la ou elle vaut `""` sur Linux et macOS pour le meme geste. `tests/broker-register-role.test.ts` distingue deliberement les deux cas (variable omise vs variable explicitement vide) et verifie que les deux normalisent en `NULL` cote broker ; aucun consommateur ne doit comparer a la chaine vide.

## desktop -- les panneaux lateraux se replient avec persistance, et le repli des filtres recoit son propre glyphe

Fichiers : `desktop/src/main/index.ts`, `desktop/src/main/menu.ts`, `desktop/src/main/store.ts`, `desktop/src/main/i18n.ts`, `desktop/src/preload/index.ts`, `desktop/src/renderer/src/components/GraphView.tsx`, `desktop/src/renderer/src/components/RoadmapFilterPanel.tsx`, `desktop/src/renderer/src/components/RoadmapView.tsx`, `desktop/src/renderer/src/components/TemplatesDialog.tsx`, `desktop/src/renderer/src/components/icons.tsx`, `desktop/src/renderer/src/store.ts`, `desktop/src/renderer/src/styles.css`, `desktop/src/shared/companion.ts`, `desktop/src/shared/types.ts`, `desktop/locales/en.json`, `desktop/locales/fr.json`, `DESIGN.md`, `TESTING.md`, plusieurs `tests/desktop-*.test.ts` (deux nouveaux). Cartes `7a2e76c6`, `67c21dd5`, `290a14e2`, `6aef4c54`.

**Quatre cartes, trois commits, parce que les separer aurait produit des etats jamais testes.** Le token `--panel-rail-w` est declare par `7a2e76c6` et consomme par `67c21dd5` : le couper aurait laisse un `var()` non resolu qui tue la largeur en silence. `DEFAULT_CONFIG` porte les deux drapeaux de repli sur des lignes adjacentes, dans un hunk que git ne peut pas scinder, et le gate n'a valide que l'arbre entier. Le glyphe fusionne du dernier commit ferme `6aef4c54`, l'arbitrage de l'operateur du 2026-08-11 reste ouvert dix jours.

**Le panneau de filtres et le volet Graphe recoivent chacun un repli qui suit le patron deja pose par la Sidebar.** L'aside reste monte, une classe modificatrice le reduit au rail `--panel-rail-w` (58px, une somme RECALCULEE pour un controle solitaire et non copiee du rail Sidebar), le controle occupe le meme pixel dans les deux etats, et l'etat persiste dans `AppConfig`. Mesure a l'ecran sur instance privee : dx=0 dy=0, persistance confirmee par arret du process et relance.

**Le volet Graphe sort son bouton de creation de l'en-tete vers sa propre rangee, retiree au repli.** La tete portait trois occupants pour 189px, le libelle passait a la ligne et toute la rangee se decalait de 7,5px. La regle appliquee est celle que `Sidebar.tsx` porte deja en commentaire (carte `19f5ab5b`) : quand la place manque, la geometrie cede, jamais le texte.

**Le File menu recoit "New template..." et la modale des templates un bouton "Manage templates".** Le composeur ouvert depuis le menu recoit un compteur d'ouverture auto-effacant plutot qu'une sentinelle locale, et une cle de rendu qui force le remontage : sans elle, un second "New template" laissait le brouillon precedent a l'ecran alors que la carte promet un template vierge. Defaut pre-existant sur le bouton du mode gestion, couvert par le meme nonce.

**Le glyphe du repli de filtres est un entonnoir tire du hamburger, pas un chevron miroir.** Les deux sont la MEME primitive : `IconMenu` est trois barres horizontales egales, l'entonnoir est ces memes barres avec une conicite (10, 6, 2). Le volet Graphe garde le glyphe `panelFold` d'origine, parce qu'une liste de conversations n'est pas un filtre : le geste reste identique d'une vue a l'autre, seul l'objet differe. Mesure a l'ecran, deux etats et deux themes : glyphe rendu 13x13, bouton 33x27, identiques aux valeurs d'avant, donc la somme du token `--panel-rail-w` reste intacte. Controle immobile, dx=0 dy=0, trait en `currentColor`, aucune couleur codee dans le SVG. Derogation declaree sur le chevron associe : sa chaine differe de la paire d'origine par une translation de +4 en x, question de composition et non de place, pour rester optiquement de la meme taille que ses voisins de rail.

**`DESIGN.md` et `TESTING.md` actent ce que les trois commits de code ont appris.** Le panneau de filtres et le volet Graphe prennent leur place de freres de la Sidebar dans `DESIGN.md`, avec le token partage et sa somme ; la bande chronologique (`showTimeline`) est nommee separement comme la derniere instance du patron pre-standard encore vivante. `TESTING.md` nomme la classe de defauts que la suite ne peut pas couvrir : ni jsdom ni happy-dom n'ont de moteur de mise en page, donc `getBoundingClientRect`, `offsetHeight` et `offsetTop` y sont neutralises -- mesure a l'appui, zero usage dans tout `tests/`. Trois defauts reels de ce lot ont ete trouves uniquement a l'ecran, dont une reouverture du composeur sur un brouillon perime, un controle decale de 7,5px et un bouton 4px trop haut ; une assertion sur la regle CSS aurait produit un temoin vert dans les deux mondes.

**Gate avant commit : 2231 pass / 2 skip / 0 fail sur 194 fichiers, smoke build 241 modules, typecheck desktop propre.**

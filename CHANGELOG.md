...
**Le precedent existait, documente au long, et il a ete copie a moitie.** `tests/desktop-tile-area.test.ts` prend un instantane des descripteurs de `globalThis` avant d'enregistrer happy-dom puis les restaure tous sauf neuf noms lies au dispatch d'evenements, et son commentaire nomme explicitement le risque pour les suites broker. Le nouveau test avait repris de lui la forme du pont React et ses imports dynamiques, mais pas sa restauration. Le correctif retenu est l'autre forme legitime, l'API `unregister()` de la bibliotheque, attendue : elle restaure a la SORTIE du fichier, ce qui suffit ici, la ou tile-area a besoin des globals natifs PENDANT son propre test. `tests/desktop-happy-dom-teardown.test.ts` garde desormais les deux formes. Il parcourt l'arbre au lieu de nommer des fichiers, parce que le prochain fautif n'existe pas encore, et il porte deux assertions plancher contre la maniere dont ce genre de garde echoue OUVERT, un parcours qui ne trouve plus rien restant vert sur l'ensemble vide. Surtout, il ne se contente pas de lire un nom dans du texte source : une troisieme sonde verifie que `register()` remplace REELLEMENT `globalThis.fetch` avant de verifier que `unregister()` le rend, faute de quoi l'assertion de restauration deviendrait vraie a vide le jour ou happy-dom cesserait de remplacer fetch. Morsure prouvee par mutation avant livraison, l'effet retire faisant rougir la garde, puis restaure.

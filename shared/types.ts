...
export interface RoadmapReorderRequest {
  ...
  ids: string[];
  /**
   * Required: grouping of `ids` into queue-tie "waves" -- items in the same
   * wave share one queue position (parallel, no forced order between them);
   * flat(waves) in order must equal `ids` exactly. Send `[]` alongside an
   * empty `ids` to clear the queue.
   */
  waves: string[][];
}

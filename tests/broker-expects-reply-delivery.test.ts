// spec_258af6eb -- card 3d3c7d40 volet A, review round 2.
//
// WHY THIS FILE EXISTS. tests/peer-message-framing.test.ts proves the pure
// DECISION (shared/message-framing.ts) and nothing else. Review measured that
// removing the second argument at server.ts's send_message call site, or
// deleting the `expects_reply` block from its inputSchema, left that suite at
// full green and the smoke build passing: the module was guarded, its WIRING
// was attested by nobody. That is the repo's recurring "declared, multiplexed,
// tested, and fed by nothing" shape.
//
// A source scan over server.ts would have closed the disconnection without
// proving the contract, so this is an END-TO-END test instead: a real broker,
// two real `bun server.ts` peers, a real `tools/call send_message`, and the
// assertion made on THE TEXT THE RECIPIENT ACTUALLY RECEIVES via its own
// check_messages tool. That is literally the card's acceptance criterion
// ("prove it on the rendered text, not on the stored field"), which until now
// was satisfied only at the level of a pure function.
//
// NAMING AND CI. Prefixed `broker-` because it spawns a daemon and binds ports.
// That prefix is an EXEMPTED FAMILY in tests/desktop-ci-glob-coverage.test.ts,
// so this file is deliberately NOT collected by the workflow's pure-module glob
// -- same as every other integration suite here (broker-*, server-*,
// approval-hook, mcp-roadmap-ack). It runs in the full local `bun test` gate
// before a commit. Naming it `peer-*` to force CI collection would put a
// port-binding suite into the pure-module matrix and trip that very audit.
//

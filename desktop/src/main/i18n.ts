...
export const EN_DEFAULTS: Record<string, string> = {
  ...
  'banner.replicaOffline': 'Upstream broker not responding — working offline, {count} change(s) pending',
  'banner.replicaStale': 'Upstream broker is running an older version — update it to enable replication, {count} change(s) pending',
  'banner.replicaRefused': 'Upstream broker refuses this replica — enable serve_replicas (and a shared broker_token) on it, {count} change(s) pending',
  'banner.roadmapConflicts': '{count} roadmap conflict(s) to arbitrate',
  'banner.openRoadmap': 'Open the roadmap',
  'banner.pushRefused': '{count} change(s) refused by the upstream broker',
  ...
  'browser.annotateRegionLabel': 'Region',
  'browser.drawToolFreehand': 'Freehand',
  'browser.drawToolCircle': 'Circle',
  'browser.drawHoldHint': 'Hold Ctrl (Cmd on macOS) over the page to draw; each stroke becomes a review item',
  'toast.regionPinned': 'Region added to the review',
  'browser.pickContextCard': 'Create a card',
  'browser.annotateCreateCards': 'Create cards',
  'browser.annotateCreateCardsConfirm': 'Create {n} roadmap card(s) from this review?',
  'toast.reviewCardsCreated': '{n} card(s) created in the roadmap',
  ...
  'roadmap.filter.triage': 'Triage',
  'roadmap.filter.untriaged': 'Never triaged: {count}',
  ...
  'roadmap.triage.none': 'Not triaged',
  'roadmap.triage.needs-triage': 'Needs triage',
  'roadmap.triage.needs-info': 'Needs info',
  'roadmap.triage.ready-for-agent': 'Ready for agent',
  'roadmap.triage.ready-for-human': 'Ready for human',
  'roadmap.triage.wontfix': "Won't fix",
  'roadmap.value': 'value',
  ...
  'toast.queueReplaced': '{n} local queue position(s) replaced by the remote broker\'s order',
  ...
  'git.groupWorktrees': 'Worktrees (summed on the rail)',
  'git.groupSessions': 'Sessions outside a worktree (not counted)',
  ...
  'roadmap.wf.hint':
    'Drag roadmap cards here; agents process the chain left to right, one wave at a time -- the next wave leaves once the previous one is done.',
  'roadmap.wf.manualHint':
    'Queued cards only move when a wave is sent: press {action} to send this one now -- the Deck then advances the queue as each wave finishes.',
  'roadmap.wf.manualHintNoLead':
    'Queued cards only move when a wave is sent, and no live team-lead can receive it: give the laurel badge to a running agent in the Agents sidebar, then press {action}.',
  ...
  'roadmap.sync.conflictBadge': 'Conflict',
  'roadmap.sync.conflictHint': 'Replication conflict: this card changed here and upstream — click the badge to arbitrate',
  'roadmap.sync.remoteLockHint': 'Remote lock — {name}',
  'roadmap.sync.contestedHint': 'Locked here, and also held upstream — {name}',
  'roadmap.sync.railBadge': '{count} replication conflict(s) to arbitrate',
  'roadmap.sync.resolve': 'Resolve the conflict',
  'roadmap.sync.dialogTitle': 'Replication conflict',
  'roadmap.sync.dialogIntro': 'This card changed both here and on the upstream broker. Only the fields that differ are listed.',
  'roadmap.sync.noBase': 'This card had never been synchronized: there is no common base to compare against.',
  'roadmap.sync.noDiff': 'No content field differs any more; either side closes the conflict.',
  'roadmap.sync.colField': 'Field',
  'roadmap.sync.colLocal': 'My version',
  'roadmap.sync.colRemote': 'Broker version',
  'roadmap.sync.colBase': 'Common base',
  'roadmap.sync.changedHere': 'changed here',
  'roadmap.sync.changedUpstream': 'changed upstream',
  'roadmap.sync.lifecycle': 'lifecycle',
  'roadmap.sync.chooseRemote': 'Take the broker version',
  'roadmap.sync.chooseRemoteHint': 'The upstream content replaces this card; the local edits are dropped.',
  'roadmap.sync.chooseLocal': 'Push my version',
  'roadmap.sync.chooseLocalHint': 'The local content is kept and pushed upstream on the next pass.',
  'roadmap.sync.chooseMerge': 'Merge and reopen',
  'roadmap.sync.chooseMergeHint': 'Field-by-field merge, then the card returns to an open status.',
  'roadmap.sync.valueEmpty': '(empty)',
  'roadmap.sync.valueNone': '(none)',
  'roadmap.sync.valueYes': 'yes',
  'roadmap.sync.valueNo': 'no',
  'roadmap.sync.field.kind': 'Kind',
  'roadmap.sync.field.title': 'Title',
  'roadmap.sync.field.description': 'Description',
  'roadmap.sync.field.rationale': 'Rationale',
  'roadmap.sync.field.context': 'Context',
  'roadmap.sync.field.priority': 'Priority',
  ...
  'roadmap.sync.field.directive': 'Directive',
  'roadmap.sync.field.target_peer_ids': 'Target sessions',
  'roadmap.sync.field.inactive': 'Set aside',
  ...
  'settings.catBroker': 'Broker',
  'settings.brokerTokenMissing': 'Replication requires a bearer token shared with the upstream broker; without it the replica will report the upstream as unreachable.',
  'settings.brokerLoading': 'Reading the claude-peers configuration…',
  'settings.brokerUnavailable': 'The claude-peers configuration could not be read',
  'settings.brokerUnavailableHelp': 'The file could not be read or parsed; the detail is in the application journal. The broker settings stay unavailable until it can be read again.',
  'settings.brokerRetry': 'Read again',
  'settings.brokerHostOnly': 'Broker configuration is available on the host machine only',
  'settings.brokerHostOnlyHelp': 'This claude-peers file decides which broker every agent on the host talks to. It is read and changed there, never from a paired companion.',
  'settings.brokerMode': 'Resolved mode',
  'settings.brokerModeHelp': 'Derived from the claude-peers configuration file shared with the agent sessions and the CLI; the environment variables below override it. Nothing is picked here: local versus remote follows broker_url in that file, and the replica opt-in just below is the only part of the shape this page can change.',
  'settings.brokerModeCurrent': 'current',
  'settings.brokerModeLocal': 'Local',
  'settings.brokerModeLocalHelp': 'No remote broker: everything stays on this machine\'s loopback broker.',
  'settings.brokerModeRemote': 'Remote',
  'settings.brokerModeRemoteHelp': 'Every client talks to the remote broker directly; losing the network stops the roadmap and the messaging.',
  'settings.brokerModeReplica': 'Replica',
  'settings.brokerModeReplicaHelp': 'Clients stay on the local broker, which replicates the roadmap against the remote one in the background.',
  'settings.brokerUrl': 'Remote broker',
  'settings.brokerUrlNone': 'none configured',
  'settings.brokerUrlHelp': 'broker_url in the claude-peers configuration file.',
  'settings.brokerUrlEnv': 'Forced by the CLAUDE_PEERS_BROKER_URL environment variable.',
  'settings.brokerToken': 'Bearer token',
  'settings.brokerTokenYes': 'configured',
  'settings.brokerTokenNo': 'none',
  'settings.brokerTokenHelp': 'Presence only: the token value never leaves the main process.',
  'settings.brokerServeReplicas': 'Serves replicas',
  'settings.brokerServeReplicasYes': 'yes',
  'settings.brokerServeReplicasNo': 'no',
  'settings.brokerServeReplicasHelp': 'Whether this machine\'s broker answers another machine\'s replication requests (serve_replicas in the claude-peers configuration file, or CLAUDE_PEERS_SERVE_REPLICAS). A broker in replica mode never serves replicas, whatever this says.',
  'settings.offlineReplica': 'Replica mode (offline work)',
  'settings.offlineReplicaHelp': 'Clients (agents, CLI, this Deck) stay on this machine\'s local broker, which replicates the roadmap against the remote broker in the background. The roadmap keeps working offline and reconciles on reconnection; the conflicts are arbitrated in the Roadmap view. In this version, messaging between agents stays machine-local for as long as replica mode is on, even while online: the agents of the other machines are never reachable. The change applies to the sessions and the brokers started AFTER saving — Kory cannot restart the local broker already running.',
  'settings.offlineReplicaEnv': 'Forced by the CLAUDE_PEERS_OFFLINE_REPLICA environment variable: the file cannot decide the mode while it is set.',
  'settings.offlineReplicaNoUrl': 'Requires a remote broker URL: without broker_url there is nothing to replicate against.',
  ...
  'settings.clodexAutoStart': 'Start the clodex proxy with Kory',
  'settings.clodexAutoStartHelp':
    'Launches `clodex server --proxy` at startup when no server answers, and attempts to give the lease back when Kory closes.',
  'settings.clodexAutoStartAbsent':
    'clodex is not installed on this machine, so there is no proxy for Kory to start here.',
  'settings.clodexProxyArgs': 'Extra clodex proxy arguments',
  'settings.clodexProxyArgsPlaceholder': '--ws-diagnostics',
  'settings.clodexProxyArgsHelp':
    'Only `--ws-diagnostics` and `--no-discovery` are allowed, separated by spaces. Applied when the next proxy starts; a running proxy is unchanged. A new Clodex flag is refused until the Deck allow-list is updated. Diagnostics may log redacted headers and OpenAI usage reports verbatim.',

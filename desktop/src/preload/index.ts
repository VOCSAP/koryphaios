...
const api: DeckApi = {
  ...
  roadmapReorder: (ids: string[], waves?: string[][]) =>
    ipcRenderer.invoke('roadmap:reorder', ids, waves),
  ...
  resolveRoadmapConflict: (id: string, choice: RoadmapSyncResolution) =>
    ipcRenderer.invoke('roadmapSync:resolve', id, choice),
  ...
  inboxDelete: (ids: number[]) => ipcRenderer.invoke('inbox:delete', ids),
  ...
  approvalEnrolmentApply: (payload: unknown) =>
    ipcRenderer.invoke('approvals:enrolment-apply', payload),
  ...
  onInboxCleared: (cb: () => void) => subscribe('inbox:cleared', () => cb()),
